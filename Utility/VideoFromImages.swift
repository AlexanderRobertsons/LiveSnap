...
class VideoFromImages {
    ...
    func writeMovieToURL(url: URL, completion: @escaping (Bool) -> ()) {
        ...
        if #available(iOS 11.0, *) {
            writerInput = AVAssetWriterInput(mediaType: .video, outputSettings: ["AVVideoCodecKey": AVVideoCodecType.h264,"AVVideoWidthKey": Int(images[0].size.width),"AVVideoHeightKey": Int(images[0].size.height)])
        } else {
            writerInput = AVAssetWriterInput(mediaType: .video, outputSettings: ["AVVideoWidthKey": Int(images[0].size.width),"AVVideoHeightKey": Int(images[0].size.height)])
        }
